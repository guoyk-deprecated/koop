...
func init() {
	knownResources = append(knownResources, &Resource{
		...
		List: func(ctx context.Context, client *kubernetes.Clientset, namespace string) (names []string, err error) {
			var items *networkingv1.IngressList
			if items, err = client.NetworkingV1().Ingresses(namespace).List(ctx, metav1.ListOptions{}); err != nil {
				return
			}
		...
		GetJSON: func(ctx context.Context, client *kubernetes.Clientset, namespace, name string) (data []byte, err error) {
			...
			if obj, err = client.NetworkingV1().Ingresses(namespace).Get(ctx, name, metav1.GetOptions{}); err != nil {
				return
			}
			obj.Kind = "Ingress"
		...
		SetJSON: func(ctx context.Context, client *kubernetes.Clientset, namespace, name string, data []byte) (err error) {
			...
			if current, err = client.NetworkingV1().Ingresses(namespace).Get(ctx, name, metav1.GetOptions{}); err != nil {
			...
			if _, err = client.NetworkingV1().Ingresses(namespace).Update(ctx, &obj, metav1.UpdateOptions{}); err != nil {
				if errors.IsNotFound(err) {
					obj.ResourceVersion = ""
					if _, err = client.NetworkingV1().Ingresses(namespace).Create(ctx, &obj, metav1.CreateOptions{}); err != nil {
						return
					}
				}
				return
